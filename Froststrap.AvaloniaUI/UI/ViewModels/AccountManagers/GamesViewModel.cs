...
namespace Froststrap.UI.ViewModels.AccountManagers
{
    ...
    public partial class GamesViewModel : ObservableObject
    {
        ...
        private async Task ShowPrivateServers()
        {
            try
            {
                await Dispatcher.UIThread.InvokeAsync(() => IsPrivateServersModalOpen = true);
                await PopulatePrivateServersAsync();
            }
            catch (Exception ex)
            {
                App.Logger.WriteLine($"{LOG_IDENT}::ShowPrivateServers", $"Exception: {ex.Message}");
                await Dispatcher.UIThread.InvokeAsync(() => IsPrivateServersModalOpen = false);
            }
        }
        ...
        {
            Dispatcher.UIThread.Invoke(() => IsPrivateServersModalOpen = false);
        }
